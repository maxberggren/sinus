...
global bloom
bloom_cap = 1000000
bloom_filename = 'bloomfilter_blocket.pkl'
...
class ContentSpider(Spider):
    ...
    def task_initial(self, grab, task):
        ...
        found_location = None
        for elem in grab.doc('//span[@class="area_label"]'):
            found_location = elem.text().replace("(","").replace(")","")

        if found_location:
            text = ""
            for elem in grab.doc('//div[@class="col-xs-12 body"]'):
                text += elem.text()

            random_handler = binascii.b2a_hex(os.urandom(15))[:10]
            uniqe_handler = "blocket://" + random_handler    
...
if __name__ == "__main__":
    ...
    while True:
        bot = LinkSpider("http://www.blocket.se", 
                         restrict_to="blocket.se", 
                         skip=[], 
                         thread_number=50,
