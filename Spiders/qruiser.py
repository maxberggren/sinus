...
if __name__ == '__main__':
    BASE_URL = 'https://www.qruiser.com/'
    db = dataset.connect('sqlite:///qruiser.db')
    #db.begin()
    
    doLogin() # log in with mechanize
    
    for userId in reversed(range(1151479)):
        url, data = foundBlog(getWithMechanize("https://www.qruiser.com/showblog.php?id="+str(userId)))
        if url and data:
            print url, data
