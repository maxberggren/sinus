...
def foundBlog(data):
    ...
    try:
        return soup.select("div.list a")[0].get('href'), soup.select("div#column_center div.description")[0].text
    except:
        return False, False
        
def getThread(url):
    data = getWithMechanize(url)
    soup = BeautifulSoup(data)
    #try:
    odds = soup.select("div#column_center div.list div.odd")
    odds = [post.text for post in odds]

    evens = soup.select("div#column_center div.list div.even")
    evens = [post.text for post in evens]
    
    oddusers = soup.select("div#column_center div.list div.odd div.insertmember div.link a")
    oddusers = [user.text for user in oddusers]

    evenusers = soup.select("div#column_center div.list div.even div.insertmember div.link a")
    evenusers = [user.text for user in evenusers]
    
    oddcity = soup.select("div#column_center div.list div.odd div.insertmember div.description span:first-child")
    oddcity = [city.text for city in oddcity]

    evencity = soup.select("div#column_center div.list div.even div.insertmember div.description span:first-child")
    evencity = [city.text for city in evencity]
    
    oddtimestamps = soup.select("div#column_center div.list div.odd div.datetime")
    oddtimestamps = [datetime.datetime.fromtimestamp(float(timestamp.text.split(",")[0].split("(")[1])) for timestamp in oddtimestamps]

    eventimestamps = soup.select("div#column_center div.list div.even div.datetime")
    eventimestamps = [datetime.datetime.fromtimestamp(float(timestamp.text.split(",")[0].split("(")[1])) for timestamp in eventimestamps]    
     
    posts = odds + evens
    posts = [" ".join(post.split("\n")[:-4]) for post in posts]
    users = oddusers + evenusers
    
    citys = oddcity + evencity
    citys = citys[0:len(posts)]
    
    timestamps = oddtimestamps + eventimestamps
    
    links = soup.select("ul.listpagelinks a")
    linkurls = [link.get("href") for link in links]

    links = [link.text for link in links]
    
    
    if links:
        if links[-1] == u"N\xe4sta \xbb":
            #print "!!!!"
            #print BASE_URL + linkurls[-1]
            moreposts, moreusers, morecitys, moretimestamps = getThread(BASE_URL + linkurls[-1])
            
            posts.extend(moreposts)
            users.extend(moreusers)
            citys.extend(morecitys)
            timestamps.extend(moretimestamps)
    
    return posts, users, citys, timestamps
    #except:
    #    return None
 
def insertInDB(post, user, city, timestamp): 
    alreadyInDB = db['sources'].find_one(user=user)
    if not alreadyInDB:
        db['sources'].insert(dict(user=user, 
                                  place=city))
...
if __name__ == '__main__':
    BASE_URL = 'https://www.qruiser.com/forum/discussions/'
    db = dataset.connect('sqlite:///qruiser.db')
    #db.begin()
    
    doLogin() # log in with mechanize
    
    for forumId in reversed(range(159336)):
        posts, users, citys, timestamps = getThread("https://www.qruiser.com/forum/discussions/?listlang=SE&view="+str(forumId))
        print "antal posts hämtade: "+ str(len(posts))
        print forumId
        for post, user, city, timestamp in zip(posts, users, citys, timestamps):
            #print post, user, city, timestamp
            insertInDB(post, user, city, timestamp)
