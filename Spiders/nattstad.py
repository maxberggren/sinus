...
def scrapeBlog(url, depth):
    allText = ""
    pages = getPages(url)
    pages = pages[0:depth] # only take 11 first pages
    posts = []
    timestamps = []
...
def getUser(url):
    ...
    try:
        ...
        if len(username) > 1 and len(presentation) > 2:
            print username
            db['blogs'].insert(dict(url=blogurl, username=username, 
                                    presentation=presentation, manuellStad=None))
            justposted = db['blogs'].find_one(url=blogurl)
            #print justposted['id']
            
            # här vill jag ha flera poster istället
            posts, timestamps = scrapeBlog(blogurl, 19) # gets 20 levels deep of blog data
...
if __name__ == '__main__':
    ...
    ids = reversed(range(989823)) #  989823 aerr first
