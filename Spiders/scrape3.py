...
def scrapeWithTOR(scraperFunction, SOCKS_PORT=7010):
...
def scrapeBlog(url):
    allText = ""
    pages = getPages(url)
    pages = pages[0:10] # only take 11 first
...
def getPages(url):
    ...
    pagenumbers = range(2, max(pagenumbers)) 
...
if __name__ == '__main__':
    #scrapeWithTOR(scrapeMissedConnections)
    scrapeBlog("http://www.nattstad.se/rebeccaeden")
