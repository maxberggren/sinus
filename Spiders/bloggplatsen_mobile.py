...
def getBlog(url):
    proceed = True
    i = 1
    url = url.replace("http://", "http://mobil.")
    print url
    if not passwordProtected(url):
        ...
        while proceed:
            ...
            suburl = url + "/sida-"+str(i)+"/"
            newposts, newtimestamps, raw = getPage(suburl)
            print str(raw) + " tecken"
            time.sleep(0.5)
            
            if newposts and newtimestamps:
                posts.append(newposts)
                timestamps.append(newtimestamps)
...
def getPage(url):
    print url
    timestamp2010 = datetime.datetime(2010, 1, 1, 1, 1, 1, 1)

    response = getContent(url)
    soup = BeautifulSoup(response) 
    posts = soup.select(".inlagg_text")
    posts = [post.text for post in posts]
    timestamps = soup.select(".inlagg_info")
    raw = len(soup.prettify())

    try:
        timestamps = [parse(timestamp.text.strip().split(" - ")[1].replace(u"Igår",u"").replace(u"Idag",u"")) for timestamp in timestamps]
    except:
...
if __name__ == '__main__':
    ...
    db = dataset.connect('sqlite:///bloggplatsen_mobile.db')
    ...
    for row in data:
        ...
        for url in urls:
            ...
            if len(posts) > 2:
                ...
                for post, timestamp in zip(posts, timestamps):
                    if len(post) > 300: # more than 300 chars
                        db['posts'].insert(dict(blog_id=justpostedId, 
                                                text=post, 
                                                date=timestamp))
                print "Sparar " + str(len(posts)) + " poster"
                
            else: # To few characters to be bothered
               print "Hoppar över bloggen, för få poster."            
